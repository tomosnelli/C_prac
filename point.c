...
int main(void)
{
  ...
  *pStr = '\0';                                            // Set 1st character to null
  ...
  while(true)
  {
    ...
    if(!(strncat(pStr, buf, str_size)))                        // Concatenate the line with pStr
    {
  ...
  char* pWord = strtok(pStr, delimiters);  // Find 1st word
  ...
  while(pWord)
  {
    ...
    if(new_word)                                            // Not NULL if new word
    {
      ...
      *(pWords + word_count) = malloc(word_length);         // Allocate memory for word 
      strncpy(*(pWords + word_count), pWord, word_length); // Copy to array
      *(pnWord + word_count++) = 1;                         // Increment word count
    }
    else
      new_word = true;                                      // Reset new word flag
 
   pWord = strtok(NULL, delimiters);      // Find subsequent word
 }
